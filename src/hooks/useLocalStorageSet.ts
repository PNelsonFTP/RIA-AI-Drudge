...
export function useBookmarks() {
  const { set: bookmarks, toggle, has } = useLocalStorageSet("ria-ai-report:bookmarks");
...
export function useReadLater() {
  const { set: queue, toggle, has, remove } = useLocalStorageSet("ria-ai-report:read-later");
  return { queue, toggle, has, remove };
}

// ─── Muted sources (hide every article from these outlets) ────────────────
export function useMutedSources() {
  const { set: muted, toggle, has } = useLocalStorageSet("ria-ai-report:muted-sources");
  return { muted, toggle, has };
}

// ─── Muted categories (hide entire sections site-wide) ────────────────────
export function useMutedCategories() {
  const { set: muted, toggle, has } = useLocalStorageSet("ria-ai-report:muted-categories");
  return { muted, toggle, has };
}
...
const SNAPSHOT_KEY = "ria-ai-report:article-snapshots";
